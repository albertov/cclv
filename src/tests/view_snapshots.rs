...
// ===== Focused Entry Visual Indicator Tests =====

/// Regression test for cclv-5ur.75: Verifies focused entry has distinct visual indicator.
///
/// This test ensures that when an entry is focused (via scrolling), it has a cyan index
/// prefix, while unfocused entries have a dark gray dimmed index prefix.
///
/// Requirements:
/// - US-4 DONE WHEN: "Visual indicator shows which entry is focused"
///
/// The focused entry should have its index rendered in Cyan color, making it clear
/// which entry will be toggled by Enter/Space keys.
#[test]
fn snapshot_focused_entry_visual_indicator() {
    ...
    let entries = create_test_conversation(vec![entry1, entry2]);

    // Create view state with first entry focused
    let mut view_state = ConversationViewState::new(
        None,
        None,
        entries,
    ...
    // Set focused message to entry 0 (first entry)
    view_state.set_focused_message(Some(EntryIndex::new(0)));

    // Relayout to trigger rendering with focus
    view_state.relayout(80, WrapMode::Wrap, &crate::state::SearchState::Inactive);

    let styles = MessageStyles::new();
    let mut terminal = create_terminal(80, 15);
    ...
    // Snapshot captures focused entry with cyan index
    insta::assert_snapshot!("focused_entry_cyan_indicator", output);

    // Verify the output contains both entry indices
    assert!(
        output.contains("│  1"),
        "Entry 1 index should be visible. Output:\n{output}"
    );
    assert!(
        output.contains("│  2"),
        "Entry 2 index should be visible. Output:\n{output}"
    );
}

/// Test that unfocused entry has dimmed index, focused entry has cyan index.
///
/// This is a companion test to verify the visual difference between focused
/// and unfocused entries when both are present.
#[test]
fn snapshot_focused_vs_unfocused_entry_indices() {
    ...
    let entry2 = create_test_log_entry(
        "msg-2",
        Role::Assistant,
        MessageContent::Text("Second message (focused)".to_string()),
        EntryType::Assistant,
    );
    let entry3 = create_test_log_entry(
        "msg-3",
        Role::User,
    ...
    let entries = create_test_conversation(vec![entry1, entry2, entry3]);

    // Create view state with second entry focused
    let mut view_state = ConversationViewState::new(
        None,
        None,
        entries,
    ...
    // Set focused message to entry 1 (second entry)
    view_state.set_focused_message(Some(EntryIndex::new(1)));
    ...
    // Snapshot captures middle entry focused (entry 2 should have cyan index)
    insta::assert_snapshot!("focused_entry_middle_of_three", output);
    ...
    assert!(
        output.contains("│  2"),
        "Entry 2 (focused) index should be visible. Output:\n{output}"
    );
    assert!(
        output.contains("│  3"),
        "Entry 3 index should be visible. Output:\n{output}"
    );
}
